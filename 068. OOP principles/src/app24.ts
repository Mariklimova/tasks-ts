...
class ArrayAnalyzer_24 {
    ...
    constructor(arr_24: number[]) {
        this.arr_24 = arr_24
    }

    getAverage = (): number => this.arr_24.reduce((sum, el) => sum + el, 0) / this.arr_24.length;
    getMax = (): number => Math.max(...this.arr_24)
    getMin = (): number => Math.min(...this.arr_24)
}
